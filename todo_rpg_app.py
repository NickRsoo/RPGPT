...
class ToDoRPGApp:
    ...
    def setup_ui(self):
        ...
        ui.label(f"Willkommen, {self.character.name}!")
        with ui.row():
            ui.label("Charakterstatus: ")
            self.status = ui.label(self.get_status())
    ...
    def add_quest_dialog(self):
        with ui.dialog() as dialog, ui.card():
            ui.label("Neue Quest hinzufügen")
            description = ui.input("Questbeschreibung")
            xp_reward = ui.input("XP-Belohnung", type="number")
            gold_reward = ui.input("Gold-Belohnung", type="number")
    ...
    def add_quest(self, description, xp_reward, gold_reward, dialog=None):
        ...
        with self.quest_list:
            ui.label(f"{description} - Belohnung: {xp_reward} XP, {gold_reward} Gold").classes("quest")
    ...
    def display_shop(self):
        for item_name, item_info in self.shop.items.items():
            with self.shop_items:
                ui.label(f"{item_name}: {item_info['cost']} Gold").classes("shop-item")
